...
//#include ...           --> included in common_functions.cpp
//#include "common.hpp"  --> included in common_functions.cpp
 ...
 * FUNCTIONS 
 */
// defined in common_functions.cpp
...
int main(int argc, char *argv[])
{
   ...
   while ( run_always || (time_time() - start_PIR) < MAX_RUNTIME )
   {
      ...
      /* If DET_PERIOD, ignore CONFIRM_GAP & READ_DELAY_CONFIRM ------ */
      if (DET_PERIOD > 0 && readValue){
          //printf(" filtering detection \n");         
    ...
    // if the time gap exceeds CONFIRM_GAP, then reset detection
    ...
    if ( detected ) {
        ...
        if (verbose) std::cout << "*From detection to e-mail sent: " << 1000.0*(time_time()-start) << " ms" << std::endl;
        
        if (LED) { gpioWrite(OUTPUT_PIN,0); } 
    
        // 4- Sleep for event_gap seconds
        time_sleep(EVENT_GAP); // - POST_CAPTURE); 
     ...
     // Waits between readings
      ...
      // if time gap exceeds SEC_TEST, then capture image
